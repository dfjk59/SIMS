	...
	public static Specification<Role> getSpecification(RoleQueryDTO roleQueryDTO) {
		Specification<Role> spec = new Specification<Role>() {
			public Predicate toPredicate(Root<Role> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
				...
				if (null != roleQueryDTO && !StringUtils.isEmpty(roleQueryDTO.getUserName())) {
					Predicate p1 = cb.like(root.get("userName").as(String.class),
							"%" + roleQueryDTO.getUserName() + "%");
					list.add(p1);
				}
				if (null != roleQueryDTO && !StringUtils.isEmpty(roleQueryDTO.getPassword())) {
					Predicate p2 = cb.like(root.get("password").as(String.class),
							"%" + roleQueryDTO.getPassword() + "%");
					list.add(p2);
				}
